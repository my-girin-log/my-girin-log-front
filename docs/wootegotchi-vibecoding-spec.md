...
너는 해커톤에서 빠르게 제품 데모를 완성하는 시니어 프론트엔드 엔지니어다. `우테고치`라는 모바일 우선 웹앱을 구현한다.

우테고치는 사용자가 하루 동안 메모하듯 남긴 기록을 AI가 실시간으로 되묻고, 매일 오전 6시에 그날의 날것 기록을 `Diary`로 정리해 달력에 저장하는 서비스다. 사용자는 쌓인 다이어리 기간을 선택해 “기술 블로그”, “감정 회고”, “우테코 회고” 같은 작성 방향을 고르고, 자신의 `페르소나.md` 말투와 기록 데이터를 기반으로 블로그에 올릴 수 있는 회고 글을 생성한다. 사용자의 꾸준한 기록은 기린 캐릭터의 레벨, 경험치, 상태, 성장 이미지로 보상된다.
...
- 꾸준히 기록하면 기린이 성장하고, 오래 기록하지 않으면 상태가 나빠진다.
...
7. 프론트는 생성된 페르소나 요약을 보여주고 Home으로 이동한다.
...
- Home 상단 또는 개발자용 작은 버튼으로 `오늘 기록 정리하기`를 제공한다.
...
```ts
const STORAGE_KEY = "wootegotchi-mock-db-v2";
```
